...
class MetastonesVC: UIViewController {
    ...
    var collectionTitles = [" \(kLb.all.localized)       "]
    ...
    func setupSelection() {
        productViewModel.getProductCategory { (proceed, data) in
            if proceed {
        ...
        productViewModel.getProductList(page: apiCollectionPage?[collectionCurrentIndex], sortBy: selectedSortByList?[collectionCurrentIndex], categoryId: selectedCategoryIdList?[collectionCurrentIndex], filter: selectedFilterList?[collectionCurrentIndex]) { (proceed, data) in
            ...
            if proceed {
                ...
                if self.collectionCurrentIndex < self.collectionTitles.count-1 {
                    ...
                    if self.collectionCurrentIndex > 0 {
                        if let categoryId = self.selectionList[self.collectionCurrentIndex-1]?.children?.first?.id, let categoryName = self.selectionList[self.collectionCurrentIndex-1]?.children?.first?.name {
                        ...
                        } else if let categoryMainId = self.selectionList[self.collectionCurrentIndex-1]?.id {
    ...
    @objc func segmentSelected(sender: ScrollableSegmentedControl) {
        ...
        UIView.animate(withDuration: 0.2) {
            self.stackView.subviews[1].isHidden = self.currentSelectedIndex > 0 ? self.selectionList[self.currentSelectedIndex-1]?.children?.count ?? 0 <= 0 : true
    ...
    @IBAction func allCollectionsHandler(_ sender: Any) {
        ...
        crystalsVC.titles = self.selectionList[self.currentSelectedIndex-1]?.children
        let header: CGFloat = 160
        let rowCount = (self.selectionList[self.currentSelectedIndex-1]?.children?.count ?? 0)
...
extension MetastonesVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension MetastonesVC: ListingCVCDelegate {
